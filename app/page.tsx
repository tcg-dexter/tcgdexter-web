...
async function loadRecentMatches(): Promise<RecentMatch[]> {
  try {
    ...
    // (manual single games or best-of-3 sets included) where the player
    // recorded both sides' prize counts, which is enough to call the
    // result legitimate without a full log.
    const { data: matchRows, error: matchErr } = await admin
      ...
      .or(
        "source.eq.tcg_live_log,and(prizes_taken_player.not.is.null,prizes_taken_opponent.not.is.null)"
      )
