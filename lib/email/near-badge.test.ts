...
describe("nearBadgeFor", () => {
  it("returns null when nothing is within the window", () => {
    ...
    expect(nearBadgeFor(3, 0)).toBeNull(); // 2 decks away, 10 matches away
  });
  ...
  it("nudges one match before the next match tier", () => {
    const n = nearBadgeFor(0, 9);
    expect(n).toMatchObject({ key: "matches_10", metric: "matches", remaining: 1, badgeName: "Hobbyist" });
  });

  it("works at higher tiers (49 → Dexter, 99 → Tabletop Titan)", () => {
    expect(nearBadgeFor(49, 0)).toMatchObject({ key: "decks_50", remaining: 1 });
    expect(nearBadgeFor(0, 99)).toMatchObject({ key: "matches_100", remaining: 1 });
  });
  ...
  it("picks the closer of two eligible badges; ties break to decks", () => {
    ...
    // matches strictly closer with a wider window
    expect(nearBadgeFor(3, 9, 2)).toMatchObject({ key: "matches_10", remaining: 1 });
  });
